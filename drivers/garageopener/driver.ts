...
module.exports = class GarageOpenerDriver extends Homey.Driver {

  /**
   * onInit is called when the driver is initialized.
   */
  async onInit() {
    this.log('GarageOpener has been initialized');
  }
  ...
  async onPairListDevices() {
    this.log('GarageOpener onPairListDevices called');
    ...
    return (veluxHandler as any)['products'].Products.filter((product: Product) => product.TypeID === ActuatorType.GarageOpener).map((product: Product) => ({
      name: product.Name,
      data: {
        id: product.NodeID,
      },
